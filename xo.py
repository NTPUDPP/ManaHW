...
#Playing 
p_repeat = 'Y'
while p_repeat == 'Y' or p_repeat == 'y':
     current_state = "Not yet"
     game_state = [['','',''],['','',''],['','','',]]; #2D-Array
     winner = None 
     player_order = input('Who gonna play it first (X is human or O is bot) : ')
     ...
     while current_state == 'Not yet':
        if player_id == 0: #Human
            ...
            play_move = move_on(players[player_id],game_state,player_pos)
        else: #Bot
            player_pos = input('Enter your position to place X or O (from 1-9) : ')
            play_move = move_on(players[player_id],game_state,player_pos)
